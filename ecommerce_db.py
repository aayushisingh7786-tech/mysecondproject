import sqlite3

# 1. Connect to database (it creates the file if it doesn't exist)
conn = sqlite3.connect('ecommerce.db')
# Ensure foreign key constraints are enforced (needed per-connection in SQLite)
conn.execute("PRAGMA foreign_keys = ON") 
cursor = conn.cursor()

# 2. CREATE TABLES (The Structure)
def create_tables():
    """Creates the necessary tables for the e-commerce database."""
    # USERS Table
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS users (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            name TEXT NOT NULL,
            email TEXT UNIQUE NOT NULL
        )
    ''')
    
    # CATEGORIES Table
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS categories (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            name TEXT NOT NULL UNIQUE
        )
    ''')
    
    # PRODUCTS Table (Linked to Categories via Foreign Key)
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS products (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            name TEXT NOT NULL,
            price REAL NOT NULL,
            category_id INTEGER,
            FOREIGN KEY (category_id) REFERENCES categories (id)
        )
    ''')
    
    # ORDERS Table (Linked to Users and Products via Foreign Keys)
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS orders (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            user_id INTEGER,
            product_id INTEGER,
            order_date TEXT,
            FOREIGN KEY (user_id) REFERENCES users (id),
            FOREIGN KEY (product_id) REFERENCES products (id)
        )
    ''')
    conn.commit()
    print("✅ Tables created successfully.")

# 3. INSERT DATA (CRUD - Create)
def insert_dummy_data():
    """Inserts initial dummy data into the database if tables are empty."""
    
    # Use explicit checks or ON CONFLICT clauses for robust data insertion
    
    # Add Categories
    cursor.execute("INSERT OR IGNORE INTO categories (name) VALUES ('Electronics')")
    cursor.execute("INSERT OR IGNORE INTO categories (name) VALUES ('Books')")

    # Add Users
    cursor.execute("INSERT OR IGNORE INTO users (name, email) VALUES ('Rahul', 'rahul@example.com')")
    cursor.execute("INSERT OR IGNORE INTO users (name, email) VALUES ('Priya', 'priya@example.com')")

    # Check if products already exist before inserting to avoid duplicates if the script runs multiple times
    cursor.execute("SELECT COUNT(*) FROM products")
    if cursor.fetchone()[0] == 0:
        # Add Products (Assuming Category IDs 1 and 2 exist)
        cursor.execute("INSERT INTO products (name, price, category_id) VALUES ('Laptop', 45000.00, 1)")
        cursor.execute("INSERT INTO products (name, price, category_id) VALUES ('Python Guide', 500.00, 2)")
        
        # Add an Order (Rahul bought a Laptop)
        cursor.execute("INSERT INTO orders (user_id, product_id, order_date) VALUES (1, 1, '2025-10-25')")

    conn.commit()
    print("✅ Dummy data inserted.")

# 4. QUERY DATA (CRUD - Read with JOIN)
def show_all_orders():
    """Queries and displays all orders using SQL JOIN operations."""
    print("\n--- ALL ORDERS (Using SQL JOIN) ---")
    # This query joins 3 tables to show readable names instead of just IDs
    sql = '''
        SELECT 
            users.name AS customer_name, 
            products.name AS product_name, 
            products.price, 
            orders.order_date
        FROM orders
        JOIN users ON orders.user_id = users.id
        JOIN products ON orders.product_id = products.id
    '''
    cursor.execute(sql)
    rows = cursor.fetchall()
    
    for row in rows:
        print(f"Customer: {row[0]} | Bought: {row[1]} | Price: ₹{row[2]:.2f} | Date: {row[3]}")

# RUN THE FUNCTIONS
if __name__ == "__main__":
    try:
        create_tables()
        insert_dummy_data()
        show_all_orders()
    except sqlite3.Error as e:
        print(f"An SQLite error occurred: {e}")
    finally:
        # Close connection when script finishes
        if conn:
            conn.close()
            print("\nDatabase connection closed.")
